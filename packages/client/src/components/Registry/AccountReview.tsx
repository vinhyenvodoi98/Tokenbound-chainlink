...
export default function AccountReview({sourceChain,chain,tokenContract,tokenId,salt,handleSetSourceAccount,handleDesAccount}: AccountReviewInterface) {
  const {chain : currentChain} = useNetwork();

  const { data: registry, refetch } = useContractRead({
    address: getContractAddress(sourceChain)
  ...
  return(
    <div className='rounded-full m-2 p-2 w-[420px] h-10 bg-green-200 flex justify-center items-center gap-2'>
      {registry ? (
        <div className='flex gap-2'>
          <div className='flex items-center gap-2 tooltip' data-tip={registry}>
            <img src={CHAIN_SUPPORTED[sourceChain].image} alt={CHAIN_SUPPORTED[sourceChain].name} className="w-5 h-5 rounded"/>
      ...
      )}
      {
        chain === "43113" && <>
          <p>{" -> "}</p>
          <DestinationAccountReview handleDesAccount={handleDesAccount} chain={Number(chain)} tokenContract={tokenContract} tokenId={tokenId} salt={salt}/>
        </>
      }
    </div>
  )
...
function DestinationAccountReview({chain,tokenContract,tokenId,salt,handleDesAccount}: DestinationAccountReviewInterface) {
  const { data: registry, refetch } = useContractRead({
  ...
  useEffect(() => {
    if(!!registry)
      handleDesAccount(registry as any)
  }, [registry])
  ...
  return(
    <div className=''>
      {registry ? (
        <div>
          <div className='flex items-center gap-2 tooltip' data-tip={registry}>
            <img src={CHAIN_SUPPORTED[chain].image} alt={CHAIN_SUPPORTED[chain].name} className="w-5 h-5 rounded"/>
            {shortenAddress(registry as any)}
          </div>
        </div>
      ) : (
